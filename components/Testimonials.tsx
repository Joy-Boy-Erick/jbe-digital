...
const TestimonialCard: React.FC<{ testimonial: typeof TESTIMONIALS[0]; index: number; isVisible: boolean }> = ({ testimonial, index, isVisible }) => {
    return (
        <div className={`p-8 rounded-2xl bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-slate-200 dark:border-slate-800 shadow-lg transition-all duration-500 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{ transitionDelay: `${index * 150}ms` }}>
...
const Testimonials: React.FC = () => {
    ...
    return (
        <section id="testimonials" className="py-20 lg:py-32 bg-slate-50 dark:bg-slate-950">
